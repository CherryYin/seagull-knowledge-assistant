...
<p align="center">
  A personal knowledge platform that turns files, web pages, feeds, repositories, and papers into searchable sources, notes, wiki pages, and publishable assets.
</p>
...
```text
Sources → Notes → Wiki → Assets
   ↓         ↓       ↓       ↓
 embeddings, provenance, review, explicit Keep / Save / Publish
   ↓
SQL / vector / hybrid retrieval
   ↓
DeepSeek Harness Agent workflows through the shared gateway
```

- **PKG / Seagull** owns durable knowledge, provenance, review state, authentication, and explicit writes.
- **DeepSeek Harness** owns Agent Chat, Sessions, workflows, presets, skills, tools, and runtime trajectories.
...
- **Harness-powered agent chat** — multi-turn workflows access PKG through authenticated knowledge APIs
...
- **Session ownership** — PKG validates user ownership while Harness persists runtime session state
- **Explicit durable writes** — Agent output reaches PKG only after a user Keep, Save, or Publish action
...
| Agent runtime | DeepSeek Harness (separate runtime) |
...
| `POST` | `/action/complete` | Lightweight direct completion for Note AI |
...
│   ├── completion.py       # Lightweight Note AI completion
...
web/                        # React frontend
├── src/pages/              # Knowledge management pages; Agent Chat lives in the unified Seagull UI
