...
- Present Asset editing as one continuous Markdown document with a rendered preview and selection-scoped Agent revisions; stable Blocks, revisions, Claim references, and Map projections remain an internal persistence contract rather than the primary editing UI.
...
- Link Calendar Todos to multiple Notes before or after creation, return ordered lightweight linked Note context without N+1 queries, and let Note Detail create or navigate its open Todos; deleting a Note removes only that association without deleting the Todo.
