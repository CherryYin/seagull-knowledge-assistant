...
- Project saved Asset Blocks into a revision-based Asset Outline Map with stable Block/Claim references; Map-to-Asset changes require Preview plus a fresh revision check, then apply only to local Editor dirty state while existing Save Changes remains the sole persistence path.
