...
- Browse a unified Library over Source, Note, and Wiki records through `/library` and `/library/search`; the shared result envelope exposes independent lifecycle state, Category for Source/Note, selectable tags, dates, Source media type, signed thumbnails/playback, and text/Caption/Transcript hit explanations while legacy domain and `/search` routes remain compatible.
...
- Video Sources use resumable Worker stages for transcript, scene/keyframe segmentation, per-segment Caption and text Embedding. Search results may return `start_ms`/`end_ms`; the supported `seagull-ui` frontend seeks playback to the matched timestamp. Transcript providers run in configured order: embedded subtitle tracks, optional local `faster-whisper`, then OpenAI-compatible or DashScope/Qwen remote ASR. Long audio is split into bounded local chunks before ASR; install the `media-local` project extra on Workers that enable local transcription.
...
- Present Asset editing as one continuous Markdown document with a rendered preview and selection-scoped Agent revisions; stable Blocks, revisions, Claim references, and Map projections remain an internal persistence contract rather than the primary editing UI. Asset Outline Map navigation resolves stable Block IDs back to Markdown character ranges, scrolls the continuous editor, and temporarily highlights the target without selecting replaceable text.
- Run Newsletter automation only from a saved configuration revision: dirty UI settings must save before Run, each result carries a configuration snapshot, skipped runs are recorded, and generated Assets retain the revision used. Scheduled runs consume already collected evidence; an explicit manual Run may perform a bounded NewsAPI refresh when its local lookback window has no news, normalizing comma-separated topic lists into OR queries.
...
- Link Calendar Todos to multiple Notes before or after creation, return ordered lightweight linked Note context without N+1 queries, and let Note Detail create or navigate its open Todos; Completed entries open a knowledge detail that resolves linked Notes and each Note's related Sources. Deleting a Note removes only that association without deleting the Todo.
