...
const ASSET_TYPE_OPTIONS: AssetType[] = [...MANUAL_ASSET_TYPES.map((item) => item.value), "newsletter_issue"];

export function AssetsPage() {
  const navigate = useNavigate();
  const [searchParams, setSearchParams] = useSearchParams();
  const query = searchParams.get("q") ?? "";
  const requestedAssetType = searchParams.get("type");
  const assetType: AssetType | "all" = requestedAssetType && ASSET_TYPE_OPTIONS.includes(requestedAssetType as AssetType)
    ? requestedAssetType as AssetType
    : "all";
  const requestedStatus = searchParams.get("status");
  const status: AssetStatus | "all" = requestedStatus && STATUS_OPTIONS.includes(requestedStatus as AssetStatus)
    ? requestedStatus as AssetStatus
    : "all";
  const assetsQuery = useQuery({ queryKey: ["assets"], queryFn: () => assetsApi.list({ limit: 100 }) });
  const { scrollRef, onScroll } = useRouteScrollRestoration<HTMLDivElement>("assets-list", Boolean(assetsQuery.data));
  const backTo = `/assets${searchParams.toString() ? `?${searchParams.toString()}` : ""}`;
  const detailState = useMemo(() => ({ backTo, backLabel: "Back to Assets" }), [backTo]);

  const updateFilter = (key: "q" | "type" | "status", value: string, defaultValue = "") => {
    ...
    >
      <div className="mx-auto max-w-7xl space-y-8 px-6 py-8">
        <header className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
          ...
          <div className="flex flex-wrap gap-2"><Button variant="outline" size="lg" onClick={() => navigate("/assets/newsletter-automation", { state: detailState })}><Newspaper className="mr-2 h-4 w-4" />Newsletter Automation</Button><Button size="lg" onClick={() => navigate("/assets/new", { state: detailState })}><Sparkles className="mr-2 h-4 w-4" />Create Asset</Button></div>
        </header>
        ...
        <Card className="border-primary/20 bg-gradient-to-r from-primary/10 via-primary/5 to-background">
          <CardContent className="grid gap-5 py-6 md:grid-cols-[1fr_auto] md:items-center">
            ...
            <Button variant="outline" onClick={() => navigate("/assets/new", { state: detailState })}>Open generation guide<ArrowRight className="ml-2 h-4 w-4" /></Button>
          </CardContent>
        </Card>
        ...
        <Card className="overflow-hidden border-sky-500/20">
          <CardContent className="grid gap-5 py-6 md:grid-cols-[auto_1fr_auto] md:items-center">
            ...
            <Button variant="outline" onClick={() => navigate("/assets/newsletter-automation", { state: detailState })}>Configure automation<ArrowRight className="ml-2 h-4 w-4" /></Button>
          </CardContent>
        </Card>
        ...
        <section className="space-y-4">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex flex-col gap-2 sm:flex-row">
              <div className="relative min-w-[260px]"><Search className="absolute left-3 top-2.5 h-4 w-4 text-muted-foreground" /><Input aria-label="Search Assets" className="pl-9" placeholder="Search title, brief, or content" value={query} onChange={(event) => updateFilter("q", event.target.value)} /></div>
              <label className="flex items-center gap-2 rounded-md border bg-background px-3 text-sm"><Filter className="h-4 w-4 text-muted-foreground" /><select aria-label="Asset type filter" className="h-9 bg-transparent outline-none" value={assetType} onChange={(event) => updateFilter("type", event.target.value, "all")}><option value="all">All types</option>{MANUAL_ASSET_TYPES.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}<option value="newsletter_issue">Newsletter Issue</option></select></label>
              <label className="rounded-md border bg-background px-3 text-sm"><select aria-label="Asset status filter" className="h-9 bg-transparent outline-none" value={status} onChange={(event) => updateFilter("status", event.target.value, "all")}>{STATUS_OPTIONS.map((item) => <option key={item} value={item}>{item === "all" ? "All statuses" : STATUS_LABELS[item]}</option>)}</select></label>
            </div>
          </div>
          ...
          {!assetsQuery.isLoading && !assetsQuery.isError && assets.length === 0 && <EmptyLibrary hasAssets={Boolean(assetsQuery.data?.total)} onCreate={() => navigate("/assets/new", { state: detailState })} />}

          <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
            {assets.map((asset) => <AssetCard key={asset.id} asset={asset} detailState={detailState} />)}
          </div>
        </section>
      </div>
    </div>
...
function AssetCard({ asset, detailState }: { asset: Asset; detailState: { backTo: string; backLabel: string } }) {
  ...
  return (
    <Link to={`/assets/${encodeURIComponent(asset.id)}`} state={detailState} className="group block h-full">
