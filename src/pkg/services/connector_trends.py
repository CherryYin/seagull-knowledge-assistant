...
async def collect_daily_connector_trends(*, user_ids: list[str] | None = None, trend_date: str | None = None) -> dict[str, int]:
    stats = {"users": 0, "arxiv": 0, "github": 0, "failed": 0, "arxiv_failed": 0, "github_failed": 0}
    async with async_session() as session:
        target_users = await _trend_user_ids(session, user_ids)
        for user_id in target_users:
            user_ok = False
            try:
                arxiv_items = await collect_arxiv_trends_for_user(session, user_id=user_id, trend_date=trend_date)
                stats["arxiv"] += len(arxiv_items)
                user_ok = True
                await session.commit()
            except Exception:
                stats["arxiv_failed"] += 1
                stats["failed"] += 1
                await session.rollback()
                logger.exception("arXiv trend collection failed for user %s", user_id)
            try:
