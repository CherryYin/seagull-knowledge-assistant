...
from pkg.services.cross_cutting.llm import create_model
from pkg.services.orchestration.tools import (
    agentic_rag,
    ask_human,
...
from pkg.services.orchestration.tools_document import process_document
from pkg.services.orchestration.tools_web import find_skills, web_search
...
- 如果命中了 **stable Wiki**，优先把它当作长期主题的规范背景；再用 note/source/memory 补充新证据、例外和待确认点
- 如果知识库中有相关内容，优先引用它，而不是生成通用回答
- 如果知识库不够，用 web_search 搜索互联网补充

### 1.1 Stable Wiki 优先，但不能绕过 review
- stable Wiki 代表当前较稳定的知识层，适合回答长期主题、背景知识、项目约定、概念总结类问题
- 新发现的 source/note/memory 只能作为补充证据、修正线索或 draft 输入，**不能**直接覆盖 stable Wiki
- 当新证据与 stable Wiki 不一致时，要明确写出“当前 stable Wiki 说什么 / 新证据显示什么 / 建议如何 review”，而不是直接改写成既定事实
...
### 5.1 默认只起草，不自动写入 durable knowledge
- 除非用户明确要求执行写入，否则不要自动创建或改写 Knowledge Tree、Wiki、Asset、Note 等 durable state
- 你可以提出“建议保存到哪里”，也可以输出 reviewable draft，但默认停在建议 / 草稿 / diff / checklist
- `create_memory_from_conversation` 只能在用户明确要求“记住”时使用，而且它创建的是 low-confidence、pending-review memory
- `process_document` 仅用于把你已经写好的文档内容导出成文件，不等于把内容写入 Wiki / Knowledge Tree / Asset
...
async def build_system_prompt(
    ...
    """
    from pkg.services.cross_cutting.skills import format_skills_for_prompt, load_skills_merged
...
async def create_action_agent(callback_handler=None, profile=None, model_id=None, provider_id=None) -> Agent:
    ...
    """
    from pkg.services.cross_cutting.skills import load_skill_tools, load_skills_merged
