...
test("registers a non-writing bounded Source Mind Map proposal tool", async () => {
  ...
  assert.equal(definition.parameters.properties.proposal.properties.nodes.maxItems, 80);
  assert.deepEqual(
  ...
  assert.deepEqual(
    definition.parameters.properties.input_summary.properties.chunk_summaries.items.required,
    ["chunk_id", "chunk_index", "summary"],
  );
...
test("rejects raw PDF content and references outside the bounded input", () => {
...
test("rejects unreferenced factual nodes and oversized proposals", () => {
  ...
  assert.throws(() => normalizeSourceMindMapProposal(oversized), /between 1 and 80 items/);
});
